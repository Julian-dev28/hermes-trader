...
The exact command the user repeatedly pastes (and that works reliably for them) is shown above. It includes the explicit `sleep 1` + `cd` so the new process starts in the correct working directory with a clean process table. The `--env prod --daemon` flags are informational but kept because they match the user's standardized restart ritual.
