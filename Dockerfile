...
# only needs pyproject.toml + hermes_trader/__init__.py (it reads the package
...
#   hermes_trader/              core package — server, dashboard, agents, v2,
...
# artifacts — nothing under hermes_trader/scripts/services imports it at
...
# `<repo root>/.state` at import time and does not read HERMES_STATE_DIR.
...
# here). If scheduler.py is ever made HERMES_STATE_DIR-aware, this symlink
...
ENV SESSION_LOG_PATH=/data/session-log.jsonl \
    ...
    HERMES_STATE_DIR=/data/.state \
    HERMES_POSITIONS_SNAPSHOT_FILE=/data/.positions-snapshot.json

# ── Networking ───────────────────────────────────────────────────────────────
# hermes_trader/server.py defaults HERMES_BIND to 127.0.0.1 (a 2026-07-10
...
# AI features (hermes_trader/agents/ai_brain.py research calls, the
