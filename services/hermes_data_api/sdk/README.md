# Hermes Data API — Python SDK

A small, typed client for the Hermes Data API. Bearer auth, `{"data": ...}`
...
Then use `hermes_client.py` directly (copy it into your project, or add
`services/hermes_data_api/sdk` to your `PYTHONPATH`):

```python
from hermes_client import HermesDataClient, HermesAPIError
```
...
with HermesDataClient("https://data.hermes.example", api_key="sk_live_...") as hc:
...
Every non-2xx raises `HermesAPIError`, populated from the server's
...
hc = HermesDataClient(base_url, api_key, session=httpx.Client(transport=transport, timeout=15))
```
...
```bash
python hermes_client.py --base-url http://127.0.0.1:8000 --api-key demo-token --ticker BTC
```
...
`HermesDataClient` when you want a tiny, readable dependency with sensible
