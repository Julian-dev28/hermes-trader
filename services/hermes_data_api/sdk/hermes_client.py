"""Typed Python SDK for the Hermes Data API.

A thin, dependency-light client over the public HTTP contract:

    from hermes_client import HermesDataClient

    hc = HermesDataClient("https://data.hermes.example", api_key="sk_live_...")
...
* Non-2xx responses raise :class:`HermesAPIError`, populated from the RFC7807
...
__all__ = ["HermesDataClient", "HermesAPIError"]
...
class HermesDataClient:
    """Client for the Hermes Data API.

    Parameters
    ----------
    base_url : str
        Root URL of the API, e.g. ``"https://data.hermes.example"``. A trailing
    ...
    # -- context manager --------------------------------------------------- #
    def __enter__(self) -> "HermesDataClient":
        return self
    ...
    def net_flow(self, ticker: str, date: Optional[str] = None) -> dict[str, Any]:
        ...
        LICENSED-DATA ADAPTER SLOT: this raises :class:`HermesAPIError` with
...
if __name__ == "__main__":  # pragma: no cover - tiny smoke CLI
    ...
    ap = argparse.ArgumentParser(description="Hermes Data API smoke client")
    ap.add_argument("--base-url", default="http://127.0.0.1:8000")
    ap.add_argument("--api-key", default="demo-token")
    ap.add_argument("--ticker", default="BTC")
    args = ap.parse_args()

    with HermesDataClient(args.base_url, args.api_key) as hc:
