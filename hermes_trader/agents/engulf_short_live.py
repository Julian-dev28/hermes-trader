"""Live wiring for the cross-sectional bearish-engulf SHORT edge.

Swarm-discovered (Lane C, hardened by Lane C2, 2026-06-27). Daily cross-sectional:
a coin whose last completed daily bar is a bearish FULL-BODY engulf (today red,
yesterday green, today's body fully engulfs yesterday's) tends to fall the next day.

- last completed bar = bearish full-body engulf (body-ratio >= 1.0)
- trailing dollar-volume >= floor
- SHORT next daily open, hold 1 day, wide stop (stop-insensitive 8-40%)
- NO BTC-regime gate: record in ALL regimes and TAG the regime, so the forward
  grader can isolate the up-regime excess (the one risk the cache could not resolve:
  it's a -44% down-tape, and the same-side random-short null already controls for
  generic short beta, so the +1.25% excess is real conditional timing — but no
  up-regime sample exists yet to confirm it holds when BTC trends up).

Validation (survivor universe = upper bound, and a SHORT result is conservative —
the best shorts that died are absent): SHORT-only bearish-engulf cleared a matched
random-SHORT null by +1.25%/trade (p=0.00012), both OOS halves +, orthogonal to the
live xs_momentum book (corr -0.05), corroborated by an independent harami-short cousin.
The symmetric long leg was REFUTED, so this is SHORT-ONLY.

DEFAULT SHADOW (`shadow_only:true`): records candidates to the unified shadow ledger
(see shadow_ledger.py) and allocates ZERO capital. Flip to live only after
`scripts/shadow_status.py` returns a VALIDATED verdict that holds in up-regime AND
operator sign-off. When live, orders route through maybe_execute so every safety gate
applies. Separate from rally_exhaustion / crash_continue_div_short (different trigger).
"""
...
_BOOK_NAME = "engulf_short"
_DAY_MS = 86_400_000
_TS_FILE = state_file(".engulf_short_live_ts")
_SEEN_FILE = state_file(".engulf_short_live_seen.json")
...
def _is_bearish_full_engulf(prev: Any, cur: Any, min_body_ratio: float) -> bool:
    """Today red, yesterday green, today's body fully engulfs yesterday's body, and
    today's body >= min_body_ratio * yesterday's body."""
    po, pc = _val(prev, "o"), _val(prev, "c")
    co, cc = _val(cur, "o"), _val(cur, "c")
    if not (pc > po and cc < co):            # yesterday green, today red
        return False
    if not (co >= pc and cc <= po):          # today's body engulfs yesterday's
        return False
    cur_body = abs(cc - co)
    prev_body = abs(pc - po)
    if prev_body <= 0:
        return False
    return (cur_body / prev_body) >= min_body_ratio
...
def _analysis(signal: Dict[str, Any], cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "tp_px": 0.0,
        "reasoning": (
            f"[engulf_short] bearish full-body engulf (body-ratio {signal['body_ratio']:.2f}); "
            f"trailing dvol ${signal['trailing_dvol']/1e6:.1f}M"
        ),
        "news_risk": "none",
        "ai_down": False,
...
def _candidate_signals(config: Dict[str, Any], universe, fetch_candles: Callable,
    ...
    min_body_ratio = float(config.get("min_body_ratio", 1.0))
    ...
    for m in universe or []:
        ...
        try:
            ...
            bars = []
        if len(bars) < max(3, history_bars // 2):
            continue
        sig_t = _bar_t(bars[-1])
        if sig_t:
        ...
        if dvol < min_dvol:
            continue
        if not _is_bearish_full_engulf(bars[-2], bars[-1], min_body_ratio):
            continue
        prev_body = abs(_val(bars[-2], "c") - _val(bars[-2], "o")) or 1e-9
        body_ratio = abs(_val(bars[-1], "c") - _val(bars[-1], "o")) / prev_body
        signals.append({
            "coin": coin,
            "side": "short",
            "signal_bar_t": sig_t,
...
def maybe_run(config: Dict[str, Any], universe, positions,
    ...
    cfg = config.get("engulf_short") or {}
    ...
    if shadow_only:
        ...
        log_event(rec)
        logger.info(f"[engulf-short] SHADOW btc_up={btc_up} signals={len(signals)}")
        return rec
    ...
    for sig in signals:
        ...
        try:
            ...
            if _execute_opened(result):
                ...
                logger.warning(f"[engulf-short] {coin} not recorded - executor did not open"
                               + (f": {reason}" if reason else ""))
        except Exception as exc:
            skipped["blocked"] += 1
            claims.release(coin, _BOOK_NAME)
            logger.warning(f"[engulf-short] open {coin} failed: {exc}")
    ...
    rec = {
        "event": "engulf_short", "ts": now_ms, "shadow": False, "btc_up": btc_up,
        "signals": len(signals), "opened": opened, "skipped": skipped,
        "candidates": signals[:10],
    }
    log_event(rec)
    logger.info(f"[engulf-short] LIVE btc_up={btc_up} signals={len(signals)} "
