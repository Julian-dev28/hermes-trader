"""Live wiring for the Amihud illiquidity factor rebalancer (SHADOW-first, disabled by default).

BORDERLINE edge — comment from ALPHA-PLAN.md: "+2.33%/rebal but lumpy (2/4 quarters negative)".
Only deploy with small k_per_tercile (1-2) and treat as diversifier, not primary edge.

Drives the shared factor engine (agents/vol_dispersion.py) using ``score_fn="amihud"``:
within-β-tercile rebalancer where coins are scored by Amihud illiquidity ratio =
mean(|daily ret| / daily $volume) over ``window`` days. LONG illiquid (high Amihud) / SHORT liquid
(low Amihud), within each BTC-beta tercile.

Validated direction: long-illiquid / short-liquid (illiquidity premium). BORDERLINE — lumpy,
2/4 backtest quarters negative. Wire with minimal size; watch for beta creep.
...
- Timer persisted to .amihud_ts → restart-safe.

Mirrors sortino_live.py / vol_dispersion_live.py exactly. Config key: "amihud_factor".
"""
...
def _eligible(universe: List[Dict[str, Any]], cfg: Dict[str, Any]) -> List[str]:
    ...
    af = cfg.get("amihud_factor") or {}
    floor = float(af.get("min_volume_usd", cfg.get("min_market_volume_usd", 5_000_000)) or 0)
    topn = int(af.get("universe_top_n", 50))
    elig = []
...
def _build_target_book(universe, cfg, fetch_candles) -> TargetBook:
    ...
    if not bench or len(bench) < window + 1:
        return TargetBook([], [], {}, {})
    k = int(af.get("k_per_tercile", 1))
    return rank_universe(cbc, bench, window, k, score_fn="amihud")
...
def _analysis(coin: str, side: str, amihud: float) -> Dict[str, Any]:
    ...
    return {
        ...
        "reasoning": f"[amihud_factor] {side} (amihud={amihud:.2e})",
        "news_risk": "none", "ai_down": False, "created_at": int(time.time() * 1000),
        "composite_score": 0.0, "external_alpha": "amihud_factor",
    }
...
def maybe_rebalance(config: Dict[str, Any], universe, positions,
    ...
    BORDERLINE edge — enabled=False + shadow_mode=True are both required defaults.
    Guard: enabled=False → immediate no-op. Shadow: logs only, no orders.
    """
    af = config.get("amihud_factor") or {}
    if not bool(af.get("enabled", False)):
        return None                                            # master gate — no-op when disabled

    hold_days = float(af.get("hold_days", 10))
    now = time.time()
    ...
    if not book.longs or not book.shorts:
        logger.info("[amihud-factor] no target book (too few coins or no BTC bench) — skip")
        return None
    ...
    shadow = bool(af.get("shadow_mode", True))
    log_event({"event": "amihud_rebalance", "shadow": shadow,
    ...
    logger.info(
        f"[amihud-factor]{' SHADOW' if shadow else ' LIVE'} rebalance — "
