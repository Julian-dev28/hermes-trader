...
EQUITY = Gauge("pathia_equity_usd", "Last known account equity in USD")
OPEN_POSITIONS = Gauge(
    "pathia_open_positions", "Open positions (from the loop snapshot)"
)
OPEN_NOTIONAL = Gauge(
    "pathia_open_notional_usd", "Sum of open position notional in USD"
)
UNREALIZED_PNL = Gauge(
    "pathia_unrealized_pnl_usd", "Sum of unrealized PnL across open positions in USD"
)
TRADES_TOTAL = Gauge("pathia_trades_total", "Number of recorded trades")
LIVE_MODE = Gauge("pathia_live_mode", "1 when agent mode is LIVE, 0 otherwise")
...
FEED_TRUSTWORTHY = Gauge(
    "pathia_feed_trustworthy", "1 when the last scan was trustworthy enough to "
    "trade on, 0 when entries are blocked")
DISK_FREE = Gauge("pathia_disk_free_bytes", "Free bytes on the state filesystem")
LOG_DIR_BYTES = Gauge("pathia_log_dir_bytes", "Total bytes under logs/")
BRAIN_READY = Gauge(
    "pathia_ai_brain_ready", "1 when the selected AI brain can actually run. A "
    "dead brain returns empty completions, which downstream reads as a PASS")
CAN_TRADE = Gauge(
    "pathia_can_trade", "1 when equity clears the structural minimum. LIVE mode "
    "with 0 here means the executor refuses every order")
DRAWDOWN_PCT = Gauge(
    "pathia_drawdown_pct", "Current drawdown from peak, flow-neutral where "
    "capital flows are recorded (negative)")
MAX_DRAWDOWN_PCT = Gauge(
    "pathia_max_drawdown_pct", "Worst drawdown over the risk window (negative)")
...
ALERT_EVAL_AGE = Gauge(
    "pathia_alert_eval_age_seconds",
    "Seconds since the alert evaluator last ran. Nothing is being delivered "
    "while this grows")
ALERTS_FIRING = Gauge(
    "pathia_alerts_firing", "Number of alert rules currently firing")
BACKUP_AGE = Gauge(
    "pathia_backup_age_seconds",
...
def _refresh() -> None:
    ...
    try:
        from pathia.positions_snapshot import read_snapshot
