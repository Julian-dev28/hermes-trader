...
from pathia.client.rate_limit import HL_LIMITER as _HL_LIMITER, endpoint_weight as _endpoint_weight
...
def _http_post(path: str, payload: Dict[str, Any], timeout: int = 5) -> Any:
    ...
    max_wait = float(os.environ.get("PATHIA_HL_RATE_MAX_WAIT_S", "30"))
...
_CANDLE_CACHE_TTL_S = float(os.environ.get("PATHIA_CANDLE_CACHE_TTL_S", "90"))
...
def fetch_hl_candles(
    ...
    # scan loop and starve monitor_exits. Env-tunable: PATHIA_CANDLE_RETRIES / _BACKOFF_CAP_S.
    _max_retries = int(os.environ.get("PATHIA_CANDLE_RETRIES", "6"))
    _backoff_cap = float(os.environ.get("PATHIA_CANDLE_BACKOFF_CAP_S", "8"))
