"""Shared logging/disk policy for pathia — one source of truth for
...
---------------------------------------------
Every long-running pathia process (``scripts/trading_loop.py``,
``python -m pathia.server``, ``scripts/scheduler.py``,
...
* ``configure_logging()`` — a real ``RotatingFileHandler`` helper, for the
  ...
  ``scripts/trading_loop.py`` or ``pathia/server.py`` are changed to
...
def max_bytes() -> int:
    return _env_int("PATHIA_LOG_MAX_BYTES", DEFAULT_MAX_BYTES)


def backup_count() -> int:
    return _env_int("PATHIA_LOG_BACKUP_COUNT", DEFAULT_BACKUP_COUNT)


def log_dir_max_bytes() -> int:
    return _env_int("PATHIA_LOG_DIR_MAX_BYTES", DEFAULT_LOG_DIR_MAX_BYTES)


def disk_free_warn_bytes() -> int:
    mb = os.environ.get("PATHIA_DISK_FREE_WARN_MB")
...
def disk_free_critical_bytes() -> int:
    mb = os.environ.get("PATHIA_DISK_FREE_CRITICAL_MB")
    if mb is not None and mb.strip() != "":
        try:
            return int(mb) * 1024 * 1024
        except ValueError:
...
def rotate_interval_sec() -> int:
    return _env_int("PATHIA_LOG_ROTATE_INTERVAL_SEC", DEFAULT_ROTATE_INTERVAL_SEC)


def resolve_log_dir(root: Optional[str] = None) -> Path:
    """Resolve the logs directory. ``PATHIA_LOG_DIR`` wins (tests / manual
    override); otherwise ``<repo root>/logs``."""
    override = os.environ.get("PATHIA_LOG_DIR")
    if override:
...
def configure_logging(
    ...
    *,
    ...
    redirection — see the module docstring. Every current pathia
