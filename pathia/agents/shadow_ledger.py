...
from pathia.agents.atomic_io import append_line
...
def _ledger_dir() -> str:
    """Where the ledgers live: <PATHIA_STATE_DIR>/shadow_ledger.

    Loud when it is ambiguous. PATHIA_STATE_DIR is set in .env.local, so a tool
    ...
    d = state_file(_DIR)
    if not _AMBIGUITY_WARNED and not os.environ.get("PATHIA_STATE_DIR"):
...
def grade_records(records: List[Dict[str, Any]],
    ...
    Return shape is documented as `pathia.models.types.GradeResult`
