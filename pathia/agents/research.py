...
from pathia.indicators.math import adx, atr, candle_val, ema, rsi
from pathia.models.types import BookAnalysis, Candle, WebSearchTelemetry
...
def _fetch_news(coin: str) -> str:
    ...
    try:
        from pathia.agents.news_catalyst import google_news_search
        from pathia.agents.news_catalyst import _coin_query, relevant_articles
...
def _fetch_macro_tape() -> str:
    ...
    try:
        from pathia.agents.news_catalyst import macro_headlines
...
def research(coin: str, perception: Dict[str, Any], brain: Any | None = None) -> Dict[str, Any]:
    ...
    if len(c4h) < 30:
        ...
        # (pathia/models/types.py) at construction time — mypy will
    ...
    # (pathia/models/types.py) at construction time; `analysis` (a
