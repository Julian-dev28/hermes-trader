...
from pathia.agents import universe as universe_filter
from pathia.agents.config import get_config
from pathia.agents.rebalancer_owned import state_file
from pathia.client.hl_client import fetch_all_mids, fetch_hl_candles
...
# /api/health/system, the pathia_feed_trustworthy metric, preflight_live.py —
# runs in the server or a CLI. Reading module state cross-process gave them
# `ts: 0, markets: 0` forever, so the PathiaFeedDegraded alert could never fire
...
# process so successive cycles walk the FULL universe (see PATHIA_UNIVERSE_SWEEP).
_sweep_offset = 0
...
def scan_once(
    ...
    Args:
        ...
        parallel_workers: max concurrent market scans. Defaults to
            PATHIA_SCAN_WORKERS or the static config/default of 8.
    """
    started = time.time()
    _reset_data_gaps()
    ...
    workers = int(
        parallel_workers
        or os.environ.get("PATHIA_SCAN_WORKERS")
    ...
    max_markets = int(os.environ.get("PATHIA_MAX_MARKETS", "45"))
    max_markets_hip3 = int(os.environ.get("PATHIA_MAX_MARKETS_HIP3", "18"))
    max_markets_movers = int(os.environ.get("PATHIA_MAX_MARKETS_MOVERS", "10"))
    movers_vol_floor = float(os.environ.get("PATHIA_MOVERS_VOL_FLOOR_USD", "300000"))
    ...
    hip3_movers_floor = float(os.environ.get("PATHIA_HIP3_MOVERS_FLOOR_USD", "50000"))
    ...
    # (never miss a live ripper). Pacing (PATHIA_BATCH_SLEEP) keeps us under HL's
    # ~1200 weight/min budget. PATHIA_UNIVERSE_SWEEP=0 disables (default).
    sweep_n = int(os.environ.get("PATHIA_UNIVERSE_SWEEP", "0"))
    ...
    # `workers` threads.
    batch_size = int(os.environ.get("PATHIA_BATCH_SIZE", "20"))
    batch_sleep = float(os.environ.get("PATHIA_BATCH_SLEEP", "0.3"))
    ...
    for batch_start in range(0, total, batch_size):
        ...
        with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="pathia-scan") as pool:
            futures = [
