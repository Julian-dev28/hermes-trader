#!/usr/bin/env python3
"""pathia — autonomous multi-market trading agent for Hyperliquid.

Usage:
  pathia scan               Scan all markets for triggers
  pathia research <COIN>    AI analysis on a coin
  pathia execute            Execute trade from last analysis
  pathia status             Show agent state, positions, config
  pathia trades             Show trade history
  pathia account            Show HL account state
  pathia config [K=V ...]   Show or set config values
  pathia start              Run autonomous scanning loop
  pathia stop               Stop autonomous loop
  pathia version            Show version
"""
...
def cmd_execute():
    ...
    from pathia.agents.executor import maybe_execute
...
def cmd_config(*args):
    """Show or set config."""
    from pathia.agents.config_store import read_agent_config, write_agent_config
...
def cmd_version():
    print_banner()
    print(f"  Pathia-Trader v{__version__}\n")
...
def main():
    if len(sys.argv) < 2:
        print_banner()
        print("  Usage: pathia <command> [args]")
        print()
        print("  Commands:")
    ...
    if not handler:
        ...
        print("  Run 'pathia' for available commands.\n")
        return
